...
,
  ...
  if (trace_id && trace_id.length !== 32) {
    DEBUG_BUILD && core.logger.log(`[Profiling] Invalid traceId: ${trace_id} on profiled event`);
  }
...
function isValidSampleRate(rate) {
  ...
  if (rate < 0 || rate > 1) {
    DEBUG_BUILD && core.logger.warn(`[Profiling] Invalid sample rate. Sample rate must be between 0 and 1. Got ${rate}.`);
    return false;
  }
  return true;
}
...
function isValidProfile(profile) {
  if (profile.samples.length <= 1) {
    DEBUG_BUILD &&
      ...
      core.logger.log('[Profiling] Discarding profile because it contains less than 2 samples');
    return false;
  }
...
function isValidProfileChunk(profile) {
  if (profile.samples.length <= 1) {
    DEBUG_BUILD &&
      ...
      // they are not seeing any profiling data and we cant avoid the back and forth
      // of asking them to provide us with a dump of the profile data.
      core.logger.log('[Profiling] Discarding profile chunk because it contains less than 2 samples');
    return false;
  }
...
function maybeProfileSpan(
  ...
  if (!client) {
    DEBUG_BUILD && core.logger.log('[Profiling] Profiling disabled, no client found.');
    return;
  }

  const options = client.getOptions();
  if (!options) {
    DEBUG_BUILD && core.logger.log('[Profiling] Profiling disabled, no options found.');
    return;
  }
  ...
  if (!isValidSampleRate(profilesSampleRate)) {
    DEBUG_BUILD && core.logger.warn('[Profiling] Discarding profile because of invalid sample rate.');
    return;
  }
...
class ContinuousProfiler {constructor() { ContinuousProfiler.prototype.__init.call(this);ContinuousProfiler.prototype.__init2.call(this); }
   ...
   */
   _chunkStart() {
    if (!this._client) {
      ...
      DEBUG_BUILD && core.logger.log('[Profiling] Profiler was never attached to the client.');
      return;
    }
    if (this._chunkData) {
      DEBUG_BUILD &&
   ...
   */
   _chunkStop() {
    if (_optionalChain([this, 'access', _6 => _6._chunkData, 'optionalAccess', _7 => _7.timer])) {
      ...
      DEBUG_BUILD && core.logger.log(`[Profiling] Stopping profiling chunk: ${this._chunkData.id}`);
    }
    if (!this._client) {
      DEBUG_BUILD &&
    ...
    if (!_optionalChain([this, 'access', _8 => _8._chunkData, 'optionalAccess', _9 => _9.id])) {
      DEBUG_BUILD &&
        core.logger.log(`[Profiling] Failed to collect profile for: ${_optionalChain([this, 'access', _10 => _10._chunkData, 'optionalAccess', _11 => _11.id])}, the chunk_id is missing.`);
      return;
    }
    ...
    if (profile) {
      DEBUG_BUILD && core.logger.log(`[Profiling] Sending profile chunk ${this._chunkData.id}.`);
    }

    DEBUG_BUILD && core.logger.log(`[Profiling] Profile chunk ${this._chunkData.id} sent to Sentry.`);
    ...
    if (!chunk) {
      DEBUG_BUILD && core.logger.log(`[Profiling] Failed to create profile chunk for: ${this._chunkData.id}`);
   ...
   */
   _flush(chunk) {
    ...
    if (!transport) {
      DEBUG_BUILD && core.logger.log('[Profiling] No transport available to send profile chunk.');
      return;
    }
    ...
    transport.send(envelope).then(null, reason => {
      DEBUG_BUILD && core.logger.error('Error while sending profile chunk envelope:', reason);
    });
  }
